...
sed -i -e 's/^HOOKS=.*$/HOOKS=(base udev autodetect modconf block encrypt lvm2 filesystems keyboard fsck)/' /etc/mkinitcpio.conf
...
# Configure GRUB
sed -i 's/^GRUB_CMDLINE_LINUX=.*/GRUB_CMDLINE_LINUX="cryptdevice=\/dev\/vg0\/lv_root:cryptroot root=\/dev\/mapper\/cryptroot"/' /etc/default/grub
...
echo "cryptswap /dev/vg0/lv_swap none luks" >> /etc/crypttab
echo "crypthome /dev/vg0/lv_home none luks" >> /etc/crypttab
