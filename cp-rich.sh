...
def collect_files(sources):
    ...
    for src in sources:
        ...
        if p.is_dir():
            for f in p.rglob('*'):
                if f.is_file(): all_files.append(f)
        elif p.is_file(): all_files.append(p)
        else:
...
def run_pv(src, dest_path, size, progress, global_task, file_task=None):
    ...
    with open(dest_path, 'wb') as out_f:
        proc = subprocess.Popen(pv_cmd, stdout=out_f, stderr=subprocess.PIPE, text=True)
...
def main():
    ...
    with progress:
        for src in files:
            ...
            # Decide if per-file bar is needed
            if size >= min_bytes:
