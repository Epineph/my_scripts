...
#   - Ctrl+C override: press Ctrl+C at any point to force immediate
# reboot/shutdown.
#
# Usage: $(basename "$0") -r|-s [-H hours] [-M minutes] [-S seconds] [op# tions]
...
# ─── Signal-trap Handler ──────────────────────────────────────────────# ───────
# Called on SIGINT (Ctrl+C).  Sets override flag and jumps out of the lo# op.
on_override() {
  override=1
