...
EXAMPLES:
  ...
  # Download and slow to 85% (percent), name final file explicitly:
  ytspeed --url 'https://youtu.be/VIDEO' --tempo 85 --name "Despacito-85pct"

  # Download and slow to 0.70× (decimal), to ./out:
  ytspeed -u 'https://youtu.be/VIDEO' -t 0.70 -o ./out

  # Re-encode an existing file to 95% speed:
  ytspeed -i ./input.mkv -t 95

  # Pitch down 2 semitones without changing tempo:
  ytspeed -i ./input.mkv -p -2

  # Slow to 80% and pitch up +1 semitone:
  ytspeed -i ./input.mkv -t 80 -p 1
EOF
}
...
if [[ -n "$tempo_arg" ]]; then
  if [[ "$tempo_arg" =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
    ...
    # must be >0
    awk 'BEGIN{exit !('"$tempo_dec"' > 0)}' || { echo "Error: tempo must be > 0." >&2; exit 7; }
  else
...
build_atempo_chain() {
  ...
  # Decompose t into factors in [0.5, 2.0]; product equals t.
  while awk 'BEGIN{exit !('"$t"' < 0.5)}'; do
    chain+=("atempo=0.5")
    t=$(awk 'BEGIN{printf("%.10f",'$t' / 0.5)}')
    ((iter++)); [[ $iter -gt 12 ]] && break
  done
  while awk 'BEGIN{exit !('"$t"' > 2.0)}'; do
    chain+=("atempo=2.0")
    t=$(awk 'BEGIN{printf("%.10f",'$t' / 2.0)}')
    ((iter++)); [[ $iter -gt 12 ]] && break
  done
...
tempo_pct="$(awk 'BEGIN{printf("%d", (('"${tempo_dec:-1}"'*100)+0.5))}')"
pitch_tag=""
if [[ -n "$pitch" ]]; then
  # Use p+X or p-X tag
  if awk 'BEGIN{exit !('"$pitch"' < 0)}'; then pitch_tag="p${pitch}"; else pitch_tag="p+${pitch}"; fi
fi
...
if [[ -n "$pitch" ]]; then
  ...
  ffmpeg -hide_banner -y -i "$src_file" -vn -acodec pcm_s16le "$tmp_in" || { echo "ffmpeg audio extract failed." >&2; exit 13; }

  # Apply rubberband: pitch only, or pitch+tempo
  if [[ -n "$tempo_dec" ]]; then
    rubberband -p "$pitch" -t "$tempo_dec" "$tmp_in" "$tmp_out" || { echo "rubberband failed." >&2; exit 14; }
  else
  ...
  ffmpeg -hide_banner -y -i "$src_file" -i "$tmp_out" \
