...
# show_dir_usage.sh
#
# A script that shows a directory tree (with cumulative sizes) up to DEPTH levels
# using `tree --du`, plus a sorted `du` listing. Both TARGET and DEPTH can be
# provided as command-line arguments, or you’ll be prompted if they’re missing.
#
# Usage examples:
...
Shows a directory tree (with cumulative sizes) up to DEPTH levels using 'tree --du'
and a sorted 'du' listing. Both TARGET and DEPTH can be provided as command-line
...
Examples:
  ...
  3) Provide no arguments at all, script will prompt for both TARGET and DEPTH:
     ./show_dir_usage.sh
...
if ! command -v tree &> /dev/null; then
  echo "Error: The 'tree' command is not installed. Please install it (e.g., 'sudo pacman -S tree')"
  exit 1
fi
